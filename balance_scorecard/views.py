...
##class Home(generic.ListView):
  ##  model = Info_kpi
    ...
    ##template_name = 'balance_scorecard/home.html'
    #fecha_actual= datetime.datetime.now()
    #paginate_by = 5
    #queryset = Info_kpi.objects.filter(fecha_modificacion=fecha_actual)
...
def CuadroDeMando(request,pk):
    #indicador = Acumulado_kpi.objects.filter(indicador__categoria=pk)
    indicador= {}
    for i in Acumulado_kpi.objects.filter(indicador__categoria=pk):
        acumulado = i.acumulado
        id = Indicador.objects.get(nombre_kpi=i.indicador)
        meta = id.meta_kpi
        nombre = id.nombre_kpi
        indicador = {'nombre_kpi':nombre,'meta_kpi':meta,'acumulado':acumulado}
        #i = Indicador.objects.get(categoria=pk)
        #meta = i.meta_kpi
        prom_dias = 365/12
        fecha_actual= datetime.datetime.now()
        f_dias = int(fecha_actual.strftime("%d"))
        #proyectado = round(indicador.acumulado*(prom_dias-f_dias),0)
        #porcentaje = round((100*indicador.acumulado)/meta,2)
        return render(request,'balance_scorecard/balancedscorecard.html',{'indicador':indicador,'f_dias':f_dias})
    
#def DetalleGerente(request, pk):
    #indicador = Acumulado_kpi.objects.filter(fecha__contains=FechaNow(), indicador__id=pk)
    #return render(request,'balance_scorecard/home.html',{'indicador':indicador})
        ...
        if user.is_superuser:
            ...
            if user.is_authenticated and not user.is_staff and not user.is_superuser:
                ...
                if a is None:
                    ...
                    update.save()
                    return render(request, 'balance_scorecard/home.html',{'indicador':indicador,'update':update})
        #up = Acumulado_kpi.objects.get(fecha__contains=mes_actual, indicador=pk)
        
    #Info_kpi.objects.filter(fecha_modificacion__contains='2019-09').aggregate(Sum('valor_kpi'))#devuelve el acumulado del mes y año correspondiente
    #Info_kpi.objects.filter(fecha_modificacion__contains='2019-09', indicador__nombre_kpi='madera acerrada').aggregate(Sum('valor_kpi')) #consulta a 2 tablas simultaneamente
...
class IndicadorCrearView(BSModalCreateView):
    ...
    def form_valid(self, form):
        ...
        #obj.responsable = self.request.user
        #obj.save()
        #success_url = reverse_lazy('index')
    ...
    #def get_initial(self):
        #self.initial.update({'responsable':self.request.user })
        #return self.initial    
    #def get_form_kwargs(self):
        #kwargs = super(IndicadorCrearView, self).get_form_kwargs()
        #kwargs.update({'responsable':self.request.user})
        #return kwargs
...
class IndicadorCrearInfoView(BSModalCreateView, SuccessMessageMixin):
    ...
    def form_valid(self, form):
        if form.is_valid():
            ...
            #cod = form.request.POST.get('indicador')
...
def Acumulado(request):
        if request.method == 'POST':
            ...
            if form.is_valid():
                f = form.cleaned_data['fecha_modificacion']
                v = form.cleaned_data['valor_kpi']
                form.save()
                #form2 = AcumuladoForm(fecha=f,valor_kpi=v)
                return render(request,'balance_scorecard/home.html',{'f':f})
        else:
            form = InformacionForm()
        return render(request,'balance_scorecard/crear_info.html',{'form':form})
...
def Añadir(request,pk):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            form.indicador = '18'
            form.save()
            return redirect('balance_scorecard/home.html')
    else:
        form = InformacionForm()
    return render(request,'balance_scorecard/crear_info.html',{'form':form})
